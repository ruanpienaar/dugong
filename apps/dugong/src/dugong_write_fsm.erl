...
write(N, W, Key, Data, Pid, ReqID) ->
    dugong_write_fsm_sup:start_write_fsm([ReqID, Pid, Key, Data, N, W, write]),
    {ok, ReqID}.

delete(N, Key, Pid, ReqID) ->
    dugong_write_fsm_sup:start_write_fsm([ReqID, Pid, Key, nil, N, N, delete]),
    {ok, ReqID}.
...
prepare(timeout, SD0=#state{n=N, key=Key}) ->
    ...
    Preflist = riak_core_apl:get_apl(DocIdx, N, dugong),
