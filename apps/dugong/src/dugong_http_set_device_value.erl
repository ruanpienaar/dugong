-module(dugong_http_set_device_value).
...
rest_init(Req, _Opts) ->
    ...
    {Value, Req3} = cowboy_req:binding(value, Req2),
    {Method, Req4} = method(Req3),
    {ok, Req4, #state{bucket=Bucket, key=Key, method=Method, value=Value}}.
...
resource_exists(Req, State=#state{key=undefined, method=delete}) ->
    {false, Req, State};
resource_exists(Req, State=#state{bucket=Bucket, key=Pin, method=Method, value=Value})
        when Method =:= get; Method =:= delete ->
    case dugong:Method(Bucket, Pin, Value) of
